...
<script>
import { queryClassroomList, getStuEditInfo, getClassType, tenantUserSummaryQuery, updateClass } from '@/api/nurseryClass/gradeManage'
export default {
  data() {
    /* 校验最大人数*/
  ...
  methods: {
    show(val) {
      ...
      if (val) {
        ...
        getStuEditInfo(params).then(res => {
          const data = res.data
          if (data.errorCode === 0) {
            this.dialogVisible = true
            this.stuInfo = data
            this.ruleForm.minAge = this.stuInfo.min
            this.ruleForm.maxAge = this.stuInfo.max
            this.ruleForm.clsType = this.stuInfo.clsType
            this.ruleForm.month = this.stuInfo.month
            this.ruleForm.min = this.stuInfo.min
            this.ruleForm.max = this.stuInfo.max
            this.ruleForm.clsName = this.stuInfo.clsName
            this.ruleForm.clsRoomId = this.stuInfo.clsRoomId
            this.ruleForm.number = this.stuInfo.number
            const mteachers = this.stuInfo.mteachers && this.stuInfo.mteachers.split(',')
            if (mteachers && mteachers.length > 0) {
              this.ruleForm.mTeachers = mteachers
            }
            const ateachers = this.stuInfo.ateachers && this.stuInfo.ateachers.split(',')
            if (ateachers && ateachers.length > 0) {
              this.ruleForm.aTeachers = ateachers
            }
            const nurserys = this.stuInfo.nurserys && this.stuInfo.nurserys.split(',')
            if (nurserys && nurserys.length > 0) {
              this.ruleForm.nurserys = nurserys
            }
          } else {
            this.$message.error(data.errorMessage || '获取编辑数据失败')
          }
        })
      }
    },
