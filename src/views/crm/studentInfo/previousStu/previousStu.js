...
import DistributeDialog from '../readingStu/components/distributeDialog.vue'
import TeachDistributeDialog from '../readingStu/components/teachDistributeDialog.vue'
import serviceDistributeDialog from '../readingStu/components/serviceDistributeDialog.vue' // 分配给客服
...
import ParentBindWX from '../readingStu/components/parentBindWX.vue'
import NameImportDialog from '../readingStu/components/nameImportDialog.vue'
import SubUserSelect from './../../../../components/SubUserSelect/SubUserSelect.vue'
import bindingFace from '../readingStu/components/bindingFace.vue' // 绑定人脸的弹窗
export default {
  components: {
    CommonTable,
    CommonSearch,
    ...
    return {
      ...
      formInline: {
        ...
        forms: [
          ...
          //   startPlaceholder: '最跟进开始日期',
          //   endPlaceholder: '最后跟进结束日期',
          //   valueFormat: 'yyyy-MM-dd',
          //   modelValue: 'finalFollowTime',
  ...
  methods: {
    ...
    delCheckData(val) {
      if (val > 0) {
        this.$refs.batchDelete.show(val)
      } else {
    ...
    openSideDetail(row) {
      ...
      getStuMsg(params).then(res => {
        if (res.data.errorCode === 0) {
          ...
          this.$refs.crmDetailModal.show('2')
        } else {
          this.$message.error(res.data.errorMessage)
        }
      })
    ...
    openListenDialog(row) {
      this.$refs.applyDialog.showDialog('add', row)
    },
