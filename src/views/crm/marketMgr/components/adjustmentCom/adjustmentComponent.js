...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        date: [
          { required: true, message: '请输入库存调整日期', trigger: 'change' }
        ]
      },
      addNewData: {
        date: '',
  ...
  methods: {
    ...
    deleteStockAction() {
      ...
      this.deleteStock({
        ids,
        callback: this.searchHandle.bind(this, { type: '8', pageIndex: this.$refs.tableCommon.pageIndex })
      })
    ...
    deleteTableRow(val) {
      ...
      const count = this.addNewData.allTotal - parseInt(val.total)
      this.addNewData.allTotal = count
