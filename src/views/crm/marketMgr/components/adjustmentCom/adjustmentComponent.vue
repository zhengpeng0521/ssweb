<template>
  <div class="adjustment_container">
    <!-- 搜索栏 -->
    <div class="search">
      <CommonSearch
        :is-inline="true"
    ...
    <div class="table_checked_box">
      <span class="table_checked_text">
        已选
        <span class="checked_nums">{{ (checkedData && checkedData.length) || '0' }}</span>条数据
      </span>
      <span>
    ...
    <CommonTable
      ...
      :options="options"
      table-key="crm_inventory_adjust"
    ...
    >
      ...
      >
        <div class="add_form_box">
          <el-form-item label="库存调整日期" prop="date">
            <el-date-picker
              v-model="addNewData.date"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="请输入库存调整日期"
            />
          </el-form-item>
        </div>
        <span class="tip_text">*若数量输入正数则增加库存，若输入负数则减少库存</span>
        <div class="dialog_table_box">
          <el-table :data="addDataTble" style="width: 100%" border stripe>
            ...
            <el-table-column label="进货单价" prop="purPrice" width="120">
              <template slot-scope="scope">
                <el-input-number
          ...
          <div v-if="!rowId" class="addBtn" type="text" @click="addTableRow()">
...
<script src='./adjustmentComponent.js'></script>

<style lang="scss" scoped>
.adjustment_container /deep/ {
  .el-table td,
  .el-table th.is-leaf {
    border-bottom: none;
  }
