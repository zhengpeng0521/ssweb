<template>
  <div>
    ...
    <BookingClass
      ...
      @toUpdateTable="getBookingUpdateTable"
    />
    <ArrangeSuccess
      ref="arrangeSucc"
...
import { excistHoliday, listByTime, queryCourseById, courseThemeQuery, courseSummaryQuery, tenantUserSummaryQuery, queryClassroomList, createArrange } from '@/api/teachManage/arrangeCourse'
export default {
  components: {
    BookingClass,
    ArrangeSuccess,
  ...
  mounted() {
    ...
    this.getUserlist()
    this.getCourseList()
    this.ruleForm.defaultOrgName = this.$store.getters.orgName
  },
  methods: {
    /* 关闭 */
    close() {
    ...
    /* 排课约课的刷新 */
    getUpdateForm(val) {
      this.isShowBooking = val
      this.ruleForm.defaultOrgName = this.$store.getters.orgName
    },
    /* 排课并预约的时间传递 */
    getBookingUpdateTable(start, end) {
      this.$router.push({ name: 'courseSchedule', params: { start, end }})
    },
    /* 颜色选择改变 */
    colorChange(val) {
    ...
    weekChange(value) {
      ...
      if (!!times && !!times[0] && !!times[1]) {
        list.forEach((item, key) => {
          if (value == key) { //eslint-disable-line
    ...
    getArrangeAndOrder(mainTeacherIds, assistanTeacherIds, start, end, list) {
      ...
      createArrange(params).then(res => {
        ...
        if (data.errorCode === 0) {
          ...
          this.ruleForm.month = ''
          this.$nextTick(() => {
            this.$refs.bookingClass.show(newReatable, start, end, params.courseId)
          })
          this.$refs['ruleForm'].resetFields()
        } else if (data.fields && data.fields.length > 0) {
          data.fields.map(item => {
