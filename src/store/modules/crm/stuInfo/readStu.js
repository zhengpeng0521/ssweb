...
import {
  isModelFile,
  previewData,
  importThreadStuData, //eslint-disable-line
  importComplete
} from '@/api/crm/studentInfo/readingStu'
...
const readStu = {
  ...
  actions: {
    ...
    /** 导入 */
    async importThreadStuData({ state, commit, dispatch }, payload) {
      commit(SHOW_LOADING)
      const params = {
        regex: payload.regex,
        id: payload.id
      }
      const { data } = await importThreadStuData(params)
      if (data && data.errorCode === 0) {
